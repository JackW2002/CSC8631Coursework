...
For this analysis a similar data will be used. With the addition of `x_enrolments` data. This data frame contains enrollment information on every student who participated in the program. Crucially it records pruchased_statement_at which records the date each student has purchased a certificate at. This column is left blank when no purchase was made, making it simple to see weather each student ended up purchasing a certificate after they were done with the program. Since I am still interested in if archetype data effects purchase probability data from rune 3-7 will be used so that students can be tied to their archetype.
...
This was then merged with the previously created `student_grades` data frame to create student_grades_certificates, in the 8-Create-student-certificate R script. The resulting data frame store the same grade, student id and archetype data as well as their certificate purchase date. The decision was made to use an inner join for this so that the data frame created had all the necessary data for modelling. Finally so that it was easy to determine weather a student made a certificate purchase or not a new Boolean column (certificate_purchased) was added.
